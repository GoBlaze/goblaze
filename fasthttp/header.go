...
func (h *RequestHeader) ContentType() []byte {
	if h.disableSpecialHeader {
		return peekArgBytes(h.h, StringToBytes(HeaderContentType))
	}
...
func (h *RequestHeader) Host() []byte {
	if h.disableSpecialHeader {
		return peekArgBytes(h.h, StringToBytes(HeaderHost))
	}
	return h.host
}
...
func (h *RequestHeader) UserAgent() []byte {
	if h.disableSpecialHeader {
		return peekArgBytes(h.h, StringToBytes(HeaderUserAgent))
	}
	return h.userAgent
}
...
func (h *RequestHeader) Method() []byte {
	if len(h.method) == 0 {
		return StringToBytes(MethodGet)
	}
	return h.method
}
